...
jest.mock('@/components/ui/ParticleBackground', () => ({
  ParticleBackground: () => <div data-testid="particle-background">Particles</div>,
}));

jest.mock('@/components/ui/GradientText', () => ({
  GradientText: ({ text, className, gradient }) => (
    <h1 data-testid="gradient-text" className={className} data-gradient={gradient}>
      {text}
    </h1>
  ),
}));
...
describe('HeroSection', () => {
  ...
  it('displays correct description text', () => {
    ...
    expect(screen.getByText(/Unlock the power of data-driven insights/)).toBeInTheDocument();
  });
  ...
  it('has correct CSS classes for responsive design', () => {
    const { container } = render(<HeroSection onSearch={mockOnSearch} loading={false} />);
  ...
  it('passes correct gradient prop to GradientText', () => {
    ...
    const gradientText = screen.getByTestId('gradient-text');
    expect(gradientText).toHaveAttribute('data-gradient', 'from-blue-600 via-purple-600 to-pink-600');
  });
});
