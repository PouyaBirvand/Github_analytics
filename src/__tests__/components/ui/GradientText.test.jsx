...
describe('GradientText', () => {
  ...
  it('applies custom gradient classes', () => {
    render(
      <GradientText text="Test Text" gradient="from-red-500 to-yellow-500" />
    );
    const element = screen.getByText('Test Text');
  ...
  it('applies custom className', () => {
    render(<GradientText text="Test Text" className="custom-class text-4xl" />);
    const element = screen.getByText('Test Text');
  ...
  it('handles long text with word wrapping', () => {
    ...
    expect(
      screen.getByText('This is a very long text that should wrap')
    ).toBeInTheDocument();
  ...
  it('applies break-words class for long text', () => {
    const { container } = render(
      <GradientText text="Very long text with multiple words" />
    );
